...
public final class MineoutActive {
    ...
    private void onPlayerComplete(long time, ServerPlayerEntity playerThatCompleted) {
        PlayerSet players = this.gameSpace.getPlayers();
        this.spawnSpectator( playerThatCompleted );

        this.playerStates.removeInt( playerThatCompleted.getUuid() );

        int finishSeconds = (int) (time - this.startTime) / 20;
        this.finishRecords.add( new FinishRecord( playerThatCompleted.getGameProfile(), finishSeconds ) );

        this.updateSidebar();

        Text message = new LiteralText( "Well Done!" ).formatted( Formatting.GREEN );
        Text subtitle = new LiteralText( "You completed the course in " )
                .append( String
                        .valueOf( finishSeconds ) )
                .append( "s!" )
                .formatted( Formatting.GOLD );

        playerThatCompleted.networkHandler.sendPacket( new TitleS2CPacket( 10, 60, 10 ) );
        playerThatCompleted.networkHandler.sendPacket( new TitleS2CPacket( TitleS2CPacket.Action.TITLE, message ) );
        playerThatCompleted.networkHandler.sendPacket( new TitleS2CPacket( TitleS2CPacket.Action.SUBTITLE, subtitle ) );

        players.stream().filter( stillPlayingPlayer -> !stillPlayingPlayer.getUuid().equals( playerThatCompleted.getUuid() ) )
                .forEach( stillPlayingPlayer -> {
                    stillPlayingPlayer.playSound( SoundEvents.ENTITY_VILLAGER_YES, SoundCategory.PLAYERS, 1.0F, 1.0F );
                    stillPlayingPlayer.sendMessage( new LiteralText( "" ).append( playerThatCompleted.getDisplayName() ).append( " finished with a time of " ).append( new LiteralText( finishSeconds + "s" ).formatted( Formatting.AQUA ) ).append( "!" ).formatted( Formatting.GOLD ), false );
                } );
    }
    ...
    private void broadcastFinish() {
        ...
        if (!this.finishRecords.isEmpty()) {
            ...
            players.forEach( player -> {
                ...
                Text subtitle;
                if (this.finishRecords.stream().anyMatch( finishRecord -> finishRecord.player.getId().equals( player.getUuid() ) )) {
                    FinishRecord record = this.finishRecords.stream().filter( rec -> rec.player.getId().equals( player.getUuid() ) ).findFirst().get();
                    subtitle = new LiteralText( "You finished in " ).append( ordinal( this.finishRecords.indexOf( record ) + 1 ) ).append( " place." ).formatted( Formatting.BLUE );
                } else {
